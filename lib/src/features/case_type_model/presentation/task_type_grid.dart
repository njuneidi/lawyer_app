...
import 'package:lawyer_app/src/features/case_type_model/presentation/task_type_card.dart';
...
class TasksTypeGrid extends ConsumerWidget {
  const TasksTypeGrid( {
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final taskTypesListValue = ref.watch(taskTypesProvider);
    // //debugPrint('-----${casesListValue.value.toString()} $filter');
    ////debugPrint(' $filter-----${casesListValue.value.toString()} $filter');
    final filteredTaskTypeListValue = filter == null
        ? taskTypesListValue
        : ref.watch(filteredTaskTypesProvider(filter));
    //debugPrint(
    ...
    return AsyncValueWidget<List<CaseTypeModel>>(
      value: filteredTaskTypeListValue,
      data: (taskType) => taskType.isEmpty
          ? const Center(
              child: Text('empty'),
            )
          : CasessLayoutGrid(
